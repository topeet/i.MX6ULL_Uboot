...
                                                  "rdinit=/linuxrc" 
//#define CONFIG_BOOTCOMMAND      "bootm 0x80800000 0x83800000"
...
/* add by lsb 20190903 */
#define CONFIG_DISPLAY_LCD_TYPE		"hdmi"
...
#define CONFIG_MFG_NAND_PARTITION "mtdparts=gpmi-nand:64m(boot),16m(kernel),16m(dtb0),16m(dtb1),16m(dtb2),16m(dtb3),16m(dtb4),16m(dtb5),1m(misc),-(rootfs) "
#else
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"bootargs=console=ttymxc0,115200 "  \
		...
		"ubi.mtd=10 mtdparts=gpmi-nand:4m(u-boot),2m(u-boot-env),4m(logo),10m(kernel),2m(dtb0),2m(dtb1),2m(dtb2),2m(dtb3),2m(dtb4),2m(dtb5),-(rootfs) rootwait rw\0"\
